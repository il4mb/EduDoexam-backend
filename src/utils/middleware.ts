...
export const errorHandler = (error: Error & { code: any }, _: Request, response: Response, next: NextFunction) => {

  logger.error(error);

  const firebaseErrors: { [key: string]: { status: number, message: string } } = {
...
export const tokenExtractor = (request: Request & { token?: string | null }, _: Response, next: NextFunction) => {
 ...
 */
export const userExtractor = async (request: Request & { user?: any }, response: Response, next: NextFunction) => {
  try {
 ...
 */
export const examAdminParticipant = async (req: Request & { participant?: any; user: any }, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const participant: any = participantDoc.data();
 ...
 * @returns 
 */
export const examParticipant = async (req: Request & { user?: any }, res: Response, next: NextFunction) => {
  const user = req.user;
