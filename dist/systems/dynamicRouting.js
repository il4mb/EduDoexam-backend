...
const routerMapping = async (dirPath) => {
    dirPath = dirPath
    ...
    for (const file of files) {
        try {
            ...
            const module = (await import(pathToFileURL(file).href)
                .then((module) => module.default ? { [module.default.name]: module.default, ...module } : module));
            const methods = ["GET", "POST", "PUT", "DELETE", "PATCH"];
            Object.keys(module)
                .filter((key) => typeof module[key] === "function" && methods.includes(key))
                .forEach((method) => {
                if (!routes[routePath]) {
                    routes[routePath] = {};
                }
